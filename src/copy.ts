...
export const jevCopy = {
  en: {
    ...
    locked: 'Locked',
    lockedBlurb:
      'This house is kept for one visitor. The password is not in the source — it lives as a runtime environment variable on the deployment, and it is checked on the server, where the asking costs something.',
    password: 'Password',
    unlock: 'Unlock',
    unlocking: 'Checking…',
    lockedNote: 'Without it the scene still stands; Jev just will not answer.',
    misconfigured:
      'This deployment has no JEV_PASSWORD set, so it lets nobody in. Set the variable and redeploy.',
  ...
  zh: {
    ...
    locked: '上了锁',
    lockedBlurb:
      '这座屋子只留给一位访客。口令不在源码里——它是部署上的运行时环境变量，并且在服务端校验，因为花钱的是那一头。',
    password: '口令',
    unlock: '开锁',
    unlocking: '正在核对……',
    lockedNote: '没有口令，屋子照样在，只是 Jev 不作声。',
    misconfigured: '这个部署没有设置 JEV_PASSWORD，因此谁也进不来。设好变量后重新部署。',
    state: 'Jev 收到的状态',
